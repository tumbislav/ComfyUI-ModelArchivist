...
<script lang="ts">
    ...
    let popup = $state<HTMLDivElement>(); let prefix = $state(''); let selected = $state<string[]>([]);
...
<button class="column-filter-trigger" class:column-filter-applied={columnKey in $filterStates[tab].columns}
        type="button" bind:this={button} aria-label={locale.t('ui.column_filter.filter_column', {
...
{#if isOpen}
    <div class="column-filter-dropdown"
         role="dialog"
         aria-label={locale.t('ui.column_filter.filter_column', {column: definition.label})}
         tabindex="-1"
         bind:this={popup}
         style:left={`${x}px`}
         style:top={`${y}px`}
         style:max-height={`${height}px`}>
        <span class="dialog-label">{definition.label}</span>
        {#if definition.kind === 'prefix'}
            <label class="column-filter-prefix">
        ...
        {:else if definition.kind === 'multi'}
            <label>
                ...
                {locale.t('ui.column_filter.select_all')}
            </label>
        ...
        {:else}
            {#each [['all', locale.t('dynamic.show_all')], ['true', definition.positive], ['false', definition.negative]] as [value, label]}
                <label>
                    <input type="radio" name={id} {value} bind:group={boolean} />
                    {label}
                </label>
            {/each}
        {/if}
        <div class="spaced-horizontally">
            <button class="narrow-button" onclick={apply}>
                {locale.t('ui.column_filter.apply')}
            </button>
            <button class="narrow-button" onclick={() => { applyColumn(tab, columnKey, null); cancel(); }}>
                {locale.t('ui.column_filter.clear')}
            </button>
            <button class="narrow-button" onclick={cancel}>
                {locale.t('ui.column_filter.cancel')}
            </button>
        </div>
    </div>
{/if}
